    ...
    if (body->callee() == entry_->ret_param()) { // return
        ...
        for (size_t i = 3; i < body->num_args(); i++) {
            ...
            match->addCase(case_const, case_bb);
                ...
                body->callee()->as<Continuation>()->intrinsic() == Intrinsic::Predicated) {

        auto mem = body->arg(0);
        auto mask = body->arg(1);
        auto target_bb = cont2bb(body->arg(2)->as_nom<Continuation>());
        auto over_bb = cont2bb(body->arg(3)->as_nom<Continuation>());
        assert(target_bb);
        ...
        if (mask->isa<Vector>()) {
            ...
            if (all_one) {
                ...
                emit_phi_arg(irbuilder, body->arg(2)->as_nom<Continuation>()->param(1), new_mask);
                irbuilder.CreateBr(target_bb);
            } else if (all_zero) {
                auto new_mask = emit(mask);
                auto inv_mask = irbuilder.CreateNot(new_mask);
                emit_phi_arg(irbuilder, body->arg(3)->as_nom<Continuation>()->param(1), inv_mask);
...
llvm::Value* CodeGen::emit_store(llvm::IRBuilder<>& irbuilder, const Store* store) {
    ...
    if (auto vectype = store->val()->type()->isa<VectorType>(); vectype && vectype->is_vector()) {
        auto align = module().getDataLayout().getABITypeAlign(ptr->getType()->getScalarType()->getPointerElementType());
        irbuilder.CreateMaskedScatter(emit(store->val()), ptr, align);
        return nullptr;
    } else {
...
Continuation* CodeGen::emit_rv_intrinsic(llvm::IRBuilder<>& irbuilder, Continuation* continuation) {
    ...
    emit_unsafe(body->arg(0)); // Emit mem

    Debug de = body->callee()->debug();

    auto cont = body->arg(2)->as_nom<Continuation>();

    auto analysis_mask = emit(body->arg(1));
