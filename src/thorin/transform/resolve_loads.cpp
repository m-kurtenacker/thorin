...
public:
    ...
    const Def* get_value(const Def* alloc, Def2Def& mapping) {
        ...
        if (auto global = alloc->isa<Global>()) {
            ...
            if (!global->is_mutable() && (!global->is_externally_visible() || !global->init()->isa<Bottom>()))
...
#undef CACHED
    ...
    const Def* find_slot(const Def* ptr, bool must_be_safe = true) {
        ...
        while (true) {
            while (auto bitcast = ptr->isa<Bitcast>())
                ptr = bitcast->from();
            if (ptr->isa<Global>() && !ptr->as<Global>()->is_mutable() && (!ptr->as<Global>()->is_externally_visible() || !ptr->as<Global>()->init()->isa<Bottom>()))
                return ptr;
